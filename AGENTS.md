...
- `src/cli.ts` holds only the process entry, env wiring, and CLI subcommand dispatch (`gitea-mcp init ...`); `src/server.ts` holds only MCP composition and tool/prompt/resource registration; `src/tools.ts` holds only Zod schemas; `src/gitea-client.ts` holds only the Gitea REST client; `src/gitlab-client.ts` holds only the GitLab REST client (parallel leaf mirroring the `GiteaClient` method surface; its sole lateral import is the type-only shared params/response contract from `gitea-client.ts`); `src/skills.ts` holds only the skill install logic behind the `gitea-mcp init --tool <name>` command (ships the bundled action skills and copies each into the target tool's skills directory, one folder per skill; carries the registry of supported tools).
...
- The Gitea credential (token or git credential) is sent ONLY inside the `Authorization` header of `GiteaClient.request` (and, on the GitLab platform, of `GitLabClient.request`), in one of three schemes: `Authorization: token <token>` (for `[gitea]` config tokens and `GITEA_TOKEN`), `Authorization: Basic base64(<username>:<secret>)` (for git credentials, whose `password` field may be an account password or a PAT), or `Authorization: Bearer <token>` (for GitLab — `[gitlab]` config tokens, `GITLAB_TOKEN`, and git credentials). GitLab query-string authentication (`?private_token=` / `?access_token=`) is forbidden. Secrets MUST be retrieved through git's own machinery — `git config get --url=<baseUrl> gitea.token` / `gitlab.token` and `git credential fill` subprocesses in `git-config.ts` (issue #79) — never by in-process `node:fs` parsing of secret-bearing files (`~/.git-credentials`, `.git/config` token values); that parsing is what the CodeQL `js/file-access-to-http` alert flagged, and its mere presence in the source tree reintroduces the file-read source. Subprocess stdout (which may carry the secret) MUST NOT be logged or interpolated into error messages. The scheme is chosen by the credential state machine in `credentials.ts` / `gitea-client.ts` / `gitlab-client.ts` (see `docs/architecture.md` §5.3 / §5.6). The secret MUST NOT be logged, interpolated into error messages returned to the MCP client, or echoed in tool output; the `gitea_status` / `gitlab_status` diagnostic tools expose only `secretPresent: boolean`, a masked username, and the boolean `gitAvailable` flag.
